    ...
    queue.submit([&](cl::sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_operation(transa),
                               get_rocblas_operation(transb), m, n, k, (cuDataType *)&alpha, a_,
...
#endif
GEMM_EX_LAUNCHER(cl::sycl::half, cl::sycl::half, float, rocblas_gemm_ex, HIP_R_16F, HIP_R_16F,
                 HIP_R_32F)
GEMM_EX_LAUNCHER(cl::sycl::half, cl::sycl::half, cl::sycl::half, rocblas_gemm_ex, HIP_R_16F,
                 HIP_R_16F, HIP_R_16F)
    ...
    queue.submit([&](cl::sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_side_mode(left_right),
                               ...
                               get_rocblas_diag_type(unit_diag), m, n, (cuDataType *)&alpha, a_,
                               lda, b_, ldb);
        });
    });
}
...
inline cl::sycl::event gemm(Func func, DATATYPE_A DT_A, DATATYPE_B DT_B, DATATYPE_C DT_C,
    ...
    auto done = queue.submit([&](cl::sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_operation(transa),
                               get_rocblas_operation(transb), m, n, k, (cuDataType_C *)&alpha, a_,
                               DT_A, lda, b_, DT_B, ldb, (cuDataType_C *)&beta, c_, DT_C, ldc, c_,
                               DT_C, ldc, DT_C, rocblas_gemm_algo_standard, 0,
...
#define GEMM_EX_LAUNCHER_USM(TYPE_A, TYPE_B, TYPE_C, ROCBLAS_ROUTINE, CUDADATATYPE_A,            \
    ...
    cl::sycl::event gemm(cl::sycl::queue &queue, transpose transa, transpose transb, int64_t m,  \
                         ...
                         const cl::sycl::vector_class<cl::sycl::event> &dependencies) {          \
        return gemm(ROCBLAS_ROUTINE, CUDADATATYPE_A, CUDADATATYPE_B, CUDADATATYPE_C, queue,      \
                    transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc, dependencies); \
    }

GEMM_EX_LAUNCHER_USM(half, half, float, rocblas_gemm_ex, rocblas_datatype_f16_r,
                     rocblas_datatype_f16_r, rocblas_datatype_f32_r)
GEMM_EX_LAUNCHER_USM(half, half, half, rocblas_gemm_ex, rocblas_datatype_f16_r,

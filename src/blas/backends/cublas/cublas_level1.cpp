...
#define ASUM_LAUNCHER(TYPE1, TYPE2, CUBLAS_ROUTINE)                             \
    void asum(sycl::queue &queue, int64_t n, sycl::buffer<TYPE1, 1> &x, \
              const int64_t incx, sycl::buffer<TYPE2, 1> &result) {         \
...
#define SCAL_LAUNCHER(TYPE1, TYPE2, CUBLAS_ROUTINE)                                      \
    void scal(sycl::queue &queue, int64_t n, TYPE1 a, sycl::buffer<TYPE2, 1> &x, \
              int64_t incx) {                                                            \
...
#define AXPY_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                                \
    void axpy(sycl::queue &queue, int64_t n, TYPE alpha, sycl::buffer<TYPE, 1> &x, \
              int64_t incx, sycl::buffer<TYPE, 1> &y, int64_t incy) {                  \
...
template <typename Func, typename T1, typename T2>
inline void rotg(Func func, sycl::queue &queue, sycl::buffer<T1, 1> &a,
                 sycl::buffer<T1, 1> &b, sycl::buffer<T2, 1> &c,
                 sycl::buffer<T1, 1> &s) {
    using cuDataType1 = typename CudaEquivalentType<T1>::Type;
    using cuDataType2 = typename CudaEquivalentType<T2>::Type;
    queue.submit([&](sycl::handler &cgh) {
...
#define ROTG_LAUNCHER(TYPE1, TYPE2, CUBLAS_ROUTINE)                         \
    void rotg(sycl::queue &queue, sycl::buffer<TYPE1, 1> &a,        \
              sycl::buffer<TYPE1, 1> &b, sycl::buffer<TYPE2, 1> &c, \
              sycl::buffer<TYPE1, 1> &s) {                              \
...
template <typename Func, typename T>
inline void rotm(Func func, sycl::queue &queue, int64_t n, sycl::buffer<T, 1> &x,
                 int64_t incx, sycl::buffer<T, 1> &y, int64_t incy,
...
template <typename Func, typename T>
inline void copy(Func func, sycl::queue &queue, int64_t n, sycl::buffer<T, 1> &x,
                 int64_t incx, sycl::buffer<T, 1> &y, int64_t incy) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
    overflow_check(n, incx, incy);
    queue.submit([&](sycl::handler &cgh) {
...
template <typename Func, typename T>
inline void dot(Func func, sycl::queue &queue, int64_t n, sycl::buffer<T, 1> &x,
                const int64_t incx, sycl::buffer<T, 1> &y, int64_t incy,
...
#define DOT_LAUNCHER(EXT, TYPE, CUBLAS_ROUTINE)                                         \
    void dot##EXT(sycl::queue &queue, int64_t n, sycl::buffer<TYPE, 1> &x,      \
                  const int64_t incx, sycl::buffer<TYPE, 1> &y, const int64_t incy, \
                  sycl::buffer<TYPE, 1> &result) {                                  \
...
template <typename Func, typename T>
inline void rotmg(Func func, sycl::queue &queue, sycl::buffer<T, 1> &d1,
                  sycl::buffer<T, 1> &d2, sycl::buffer<T, 1> &x1, T y1,
                  sycl::buffer<T, 1> &param) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
...
#define ROTMG_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                          \
    void rotmg(sycl::queue &queue, sycl::buffer<TYPE, 1> &d1,                 \
               sycl::buffer<TYPE, 1> &d2, sycl::buffer<TYPE, 1> &x1, TYPE y1, \
               sycl::buffer<TYPE, 1> &param) {                                    \
...
inline void iamax(Func func, sycl::queue &queue, int64_t n, sycl::buffer<T, 1> &x,
    ...
    // the device
    result.template get_access<sycl::access::mode::write>()[0] =
        std::max((int64_t)int_res_buff.template get_access<sycl::access::mode::read>()[0] - 1,
                 int64_t{ 0 });
}

#define IAMAX_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                    \
    void iamax(sycl::queue &queue, int64_t n, sycl::buffer<TYPE, 1> &x, \
               const int64_t incx, sycl::buffer<int64_t, 1> &result) {      \
...
template <typename Func, typename T>
inline void swap(Func func, sycl::queue &queue, int64_t n, sycl::buffer<T, 1> &x,
                 int64_t incx, sycl::buffer<T, 1> &y, int64_t incy) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
    overflow_check(n, incx, incy);
    queue.submit([&](sycl::handler &cgh) {
...
#define IAMIN_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                    \
    void iamin(sycl::queue &queue, int64_t n, sycl::buffer<TYPE, 1> &x, \
               const int64_t incx, sycl::buffer<int64_t, 1> &result) {      \
...
#define NRM2_LAUNCHER(TYPE1, TYPE2, CUBLAS_ROUTINE)                             \
    void nrm2(sycl::queue &queue, int64_t n, sycl::buffer<TYPE1, 1> &x, \
              const int64_t incx, sycl::buffer<TYPE2, 1> &result) {         \
...
// Level 1
template <typename Func, typename T1, typename T2>
inline sycl::event asum(Func func, sycl::queue &queue, int64_t n, const T1 *x,
...
template <typename Func, typename T>
inline sycl::event axpy(Func func, sycl::queue &queue, int64_t n, T alpha, const T *x,
...
#define AXPY_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                        \
    sycl::event axpy(sycl::queue &queue, int64_t n, TYPE alpha, const TYPE *x, \
...
#define ROTM_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                             \
    sycl::event rotm(sycl::queue &queue, int64_t n, TYPE *x, int64_t incx, TYPE *y, \
...
template <typename Func, typename T>
inline sycl::event copy(Func func, sycl::queue &queue, int64_t n, const T *x, int64_t incx,
...
template <typename Func, typename T>
inline sycl::event dot(Func func, sycl::queue &queue, int64_t n, const T *x,
...
template <typename Func, typename T1, typename T2, typename T3>
inline sycl::event rot(Func func, sycl::queue &queue, int64_t n, T1 *x, const int64_t incx,
...
#define ROTMG_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                           \
    sycl::event rotmg(sycl::queue &queue, TYPE *d1, TYPE *d2, TYPE *x1, TYPE y1,   \
...
template <typename Func, typename T>
inline sycl::event iamax(Func func, sycl::queue &queue, int64_t n, const T *x,
...
#define SWAP_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                             \
    sycl::event swap(sycl::queue &queue, int64_t n, TYPE *x, int64_t incx, TYPE *y, \
                         int64_t incy, const std::vector<sycl::event> &dependencies) {  \
...
template <typename Func, typename T>
inline sycl::event iamin(Func func, sycl::queue &queue, int64_t n, const T *x,
                             const int64_t incx, int64_t *result,
                             const std::vector<sycl::event> &dependencies) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
    overflow_check(n, incx);
...
template <typename Func, typename T1, typename T2>
inline sycl::event nrm2(Func func, sycl::queue &queue, int64_t n, const T1 *x,
                            const int64_t incx, T2 *result,
                            const std::vector<sycl::event> &dependencies) {
    using cuDataType1 = typename CudaEquivalentType<T1>::Type;
    using cuDataType2 = typename CudaEquivalentType<T2>::Type;

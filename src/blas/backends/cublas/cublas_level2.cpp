...
#define GEMV_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                              \
    void gemv(sycl::queue &queue, transpose trans, int64_t m, int64_t n, TYPE alpha, \
              sycl::buffer<TYPE, 1> &a, int64_t lda, sycl::buffer<TYPE, 1> &x,   \
...
template <typename Func, typename T>
inline void gbmv(Func func, sycl::queue &queue, transpose trans, int64_t m, int64_t n,
                 int64_t kl, int64_t ku, T alpha, sycl::buffer<T, 1> &a, int64_t lda,
                 sycl::buffer<T, 1> &x, int64_t incx, T beta, sycl::buffer<T, 1> &y,
                 int64_t incy) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
...
#define GBMV_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                                        \
    void gbmv(sycl::queue &queue, transpose trans, int64_t m, int64_t n, int64_t kl,           \
              int64_t ku, TYPE alpha, sycl::buffer<TYPE, 1> &a, int64_t lda,                   \
              sycl::buffer<TYPE, 1> &x, int64_t incx, TYPE beta, sycl::buffer<TYPE, 1> &y, \
              int64_t incy) {                                                                      \
...
template <typename Func, typename T>
inline void ger(Func func, sycl::queue &queue, int64_t m, int64_t n, T alpha,
...
#define GER_LAUNCHER(EXT, TYPE, CUBLAS_ROUTINE)                                             \
    void ger##EXT(sycl::queue &queue, int64_t m, int64_t n, TYPE alpha,                 \
                  sycl::buffer<TYPE, 1> &x, int64_t incx, sycl::buffer<TYPE, 1> &y, \
                  int64_t incy, sycl::buffer<TYPE, 1> &a, int64_t lda) {                \
...
#define HPR2_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                             \
    void hpr2(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,          \
              sycl::buffer<TYPE, 1> &x, int64_t incx, sycl::buffer<TYPE, 1> &y, \
              int64_t incy, sycl::buffer<TYPE, 1> &a) {                             \
...
template <typename Func, typename T>
inline void tbmv(Func func, sycl::queue &queue, uplo upper_lower, transpose trans,
                 diag unit_diag, int64_t n, int64_t k, sycl::buffer<T, 1> &a, int64_t lda,
...
#define TBMV_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                                \
    void tbmv(sycl::queue &queue, uplo upper_lower, transpose trans, diag unit_diag,   \
              int64_t n, int64_t k, sycl::buffer<TYPE, 1> &a, int64_t lda,             \
              sycl::buffer<TYPE, 1> &x, int64_t incx) {                                \
...
template <typename Func, typename T>
inline void tbsv(Func func, sycl::queue &queue, uplo upper_lower, transpose trans,
                 diag unit_diag, int64_t n, int64_t k, sycl::buffer<T, 1> &a, int64_t lda,
                 sycl::buffer<T, 1> &x, int64_t incx) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
    overflow_check(n, k, lda, incx);
...
#define TBSV_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                                \
    void tbsv(sycl::queue &queue, uplo upper_lower, transpose trans, diag unit_diag,   \
              int64_t n, int64_t k, sycl::buffer<TYPE, 1> &a, int64_t lda,             \
              sycl::buffer<TYPE, 1> &x, int64_t incx) {                                \
...
template <typename Func, typename T>
inline void tpmv(Func func, sycl::queue &queue, uplo upper_lower, transpose trans,
                 diag unit_diag, int64_t n, sycl::buffer<T, 1> &a, sycl::buffer<T, 1> &x,
                 int64_t incx) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
    overflow_check(n, incx);
    queue.submit([&](sycl::handler &cgh) {
...
#define TPMV_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                              \
    void tpmv(sycl::queue &queue, uplo upper_lower, transpose trans, diag unit_diag, \
              int64_t n, sycl::buffer<TYPE, 1> &a, sycl::buffer<TYPE, 1> &x,     \
              int64_t incx) {                                                            \
...
template <typename Func, typename T>
inline void tpsv(Func func, sycl::queue &queue, uplo upper_lower, transpose trans,
                 diag unit_diag, int64_t n, sycl::buffer<T, 1> &a, sycl::buffer<T, 1> &x,
                 int64_t incx) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
    overflow_check(n, incx);
...
#define TPSV_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                              \
    void tpsv(sycl::queue &queue, uplo upper_lower, transpose trans, diag unit_diag, \
              int64_t n, sycl::buffer<TYPE, 1> &a, sycl::buffer<TYPE, 1> &x,     \
              int64_t incx) {                                                            \
...
template <typename Func, typename T>
inline void trmv(Func func, sycl::queue &queue, uplo upper_lower, transpose trans,
                 diag unit_diag, int64_t n, sycl::buffer<T, 1> &a, int64_t lda,
                 sycl::buffer<T, 1> &x, int64_t incx) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
    overflow_check(n, lda, incx);
    queue.submit([&](sycl::handler &cgh) {
...
#define TRMV_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                                       \
    void trmv(sycl::queue &queue, uplo upper_lower, transpose trans, diag unit_diag,          \
              int64_t n, sycl::buffer<TYPE, 1> &a, int64_t lda, sycl::buffer<TYPE, 1> &x, \
              int64_t incx) {                                                                     \
...
template <typename Func, typename T>
inline void trsv(Func func, sycl::queue &queue, uplo upper_lower, transpose trans,
                 diag unit_diag, int64_t n, sycl::buffer<T, 1> &a, int64_t lda,
                 sycl::buffer<T, 1> &x, int64_t incx) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
    overflow_check(n, lda, incx);
...
#define TRSV_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                                       \
    void trsv(sycl::queue &queue, uplo upper_lower, transpose trans, diag unit_diag,          \
              int64_t n, sycl::buffer<TYPE, 1> &a, int64_t lda, sycl::buffer<TYPE, 1> &x, \
              int64_t incx) {                                                                     \
...
template <typename Func, typename T>
inline sycl::event gemv(Func func, sycl::queue &queue, transpose trans, int64_t m,
...
template <typename Func, typename T>
inline sycl::event gbmv(Func func, sycl::queue &queue, transpose trans, int64_t m,
...
#define GBMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                                  \
    sycl::event gbmv(sycl::queue &queue, transpose trans, int64_t m, int64_t n,          \
...
template <typename Func, typename T>
inline sycl::event ger(Func func, sycl::queue &queue, int64_t m, int64_t n, T alpha,
...
#define GER_LAUNCHER_USM(EXT, TYPE, CUBLAS_ROUTINE)                                             \
    sycl::event ger##EXT(sycl::queue &queue, int64_t m, int64_t n, TYPE alpha,          \
...
template <typename Func, typename T>
inline sycl::event hbmv(Func func, sycl::queue &queue, uplo upper_lower, int64_t n,
...
#define HEMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                                   \
    sycl::event hemv(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,         \
...
#define HER_LAUNCHER_USM(SCALAR_TYPE, DATA_TYPE, CUBLAS_ROUTINE)                                 \
    sycl::event her(sycl::queue &queue, uplo upper_lower, int64_t n,                     \
...
#define HER2_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                             \
    sycl::event her2(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,   \
                         const TYPE *x, int64_t incx, const TYPE *y, int64_t incy, TYPE *a, \
                         int64_t lda, const std::vector<sycl::event> &dependencies) {   \
...
#define HPMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                              \
    sycl::event hpmv(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,    \
...
#define HPR_LAUNCHER_USM(SCALAR_TYPE, DATA_TYPE, CUBLAS_ROUTINE)                                 \
    sycl::event hpr(sycl::queue &queue, uplo upper_lower, int64_t n,                     \
                        const SCALAR_TYPE alpha, const DATA_TYPE *x, int64_t incx, DATA_TYPE *a, \
                        const std::vector<sycl::event> &dependencies) {                      \
...
#define HPR2_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                             \
    sycl::event hpr2(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,   \
                         const TYPE *x, int64_t incx, const TYPE *y, int64_t incy, TYPE *a, \
                         const std::vector<sycl::event> &dependencies) {                \
...
template <typename Func, typename T>
inline sycl::event sbmv(Func func, sycl::queue &queue, uplo upper_lower, int64_t n,
                            int64_t k, T alpha, const T *a, int64_t lda, const T *x, int64_t incx,
                            T beta, T *y, int64_t incy,
                            const std::vector<sycl::event> &dependencies) {
    using cuDataType = typename CudaEquivalentType<T>::Type;
...
#define SBMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                                \
    sycl::event sbmv(sycl::queue &queue, uplo upper_lower, int64_t n, int64_t k,       \
                         TYPE alpha, const TYPE *a, int64_t lda, const TYPE *x, int64_t incx,  \
                         TYPE beta, TYPE *y, int64_t incy,                                     \
                         const std::vector<sycl::event> &dependencies) {                   \
...
#define SYMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                                   \
    sycl::event symv(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,         \
                         const TYPE *a, int64_t lda, const TYPE *x, int64_t incx, TYPE beta,      \
                         TYPE *y, int64_t incy,                                                   \
                         const std::vector<sycl::event> &dependencies) {                      \
...
#define SYR_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                                   \
    sycl::event syr(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,         \
...
#define SYR2_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                             \
    sycl::event syr2(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,   \
                         const TYPE *x, int64_t incx, const TYPE *y, int64_t incy, TYPE *a, \
                         int64_t lda, const std::vector<sycl::event> &dependencies) {   \
...
#define SPMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                              \
    sycl::event spmv(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,    \
                         const TYPE *a, const TYPE *x, int64_t incx, TYPE beta, TYPE *y,     \
                         int64_t incy, const std::vector<sycl::event> &dependencies) {   \
...
#define SPR_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                              \
    sycl::event spr(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,    \
...
#define SPR2_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                             \
    sycl::event spr2(sycl::queue &queue, uplo upper_lower, int64_t n, TYPE alpha,   \
                         const TYPE *x, int64_t incx, const TYPE *y, int64_t incy, TYPE *a, \
                         const std::vector<sycl::event> &dependencies) {                \
...
#define TBMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                                  \
    sycl::event tbmv(sycl::queue &queue, uplo upper_lower, transpose trans,              \
...
#define TBSV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                                  \
    sycl::event tbsv(sycl::queue &queue, uplo upper_lower, transpose trans,              \
                         diag unit_diag, int64_t n, int64_t k, const TYPE *a, int64_t lda,       \
                         TYPE *x, int64_t incx,                                                  \
                         const std::vector<sycl::event> &dependencies) {                     \
...
#define TPMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                           \
    sycl::event tpmv(sycl::queue &queue, uplo upper_lower, transpose trans,       \
...
#define TPSV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                           \
    sycl::event tpsv(sycl::queue &queue, uplo upper_lower, transpose trans,       \
                         diag unit_diag, int64_t n, const TYPE *a, TYPE *x, int64_t incx, \
                         const std::vector<sycl::event> &dependencies) {              \
...
#define TRMV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                               \
    sycl::event trmv(sycl::queue &queue, uplo upper_lower, transpose trans,           \
...
#define TRSV_LAUNCHER_USM(TYPE, CUBLAS_ROUTINE)                                               \
    sycl::event trsv(sycl::queue &queue, uplo upper_lower, transpose trans,           \
                         diag unit_diag, int64_t n, const TYPE *a, int64_t lda, TYPE *x,      \
                         int64_t incx, const std::vector<sycl::event> &dependencies) {    \

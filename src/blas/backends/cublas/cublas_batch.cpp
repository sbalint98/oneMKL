...
inline void gemm_batch(Func func, sycl::queue &queue, transpose transa, transpose transb,
                       int64_t m, int64_t n, int64_t k, T alpha, sycl::buffer<T, 1> &a,
                       int64_t lda, int64_t stride_a, sycl::buffer<T, 1> &b, int64_t ldb,
                       int64_t stride_b, T beta, sycl::buffer<T, 1> &c, int64_t ldc,
...
#define GEMM_STRIDED_BATCH_LAUNCHER(TYPE, CUBLAS_ROUTINE)                                          \
    void gemm_batch(sycl::queue &queue, transpose transa, transpose transb, int64_t m,         \
...
template <typename Func, typename T>
inline sycl::event gemm_batch(Func func, sycl::queue &queue, transpose transa,
...
template <typename Func, typename T>
inline sycl::event trsm_batch(Func func, sycl::queue &queue, side *left_right,
